...
GeometrySystem::GeometrySystem(
    ...
    if (_max_geometry_count == 0)
        Logger::fatal(GEOMETRY_SYS_LOG, "Const _max_geometry_count must be greater than 0.");
...
Geometry* GeometrySystem::acquire(const uint32 id) {
    ...
    if (ref == _registered_geometries.end()) {
        Logger::error(GEOMETRY_SYS_LOG, "Invalid geometry requested. Return nullptr.");
        return nullptr;
    }
...
void GeometrySystem::release(Geometry* geometry) {
    if (!geometry || !geometry->id.has_value()) {
        Logger::warning(GEOMETRY_SYS_LOG, "Cannot release invalid geometry. Nothing was done.");
        return;
    }

    auto& ref = _registered_geometries[geometry->id.value()];
    if (ref.handle->id != geometry->id.value()) {
        Logger::fatal(GEOMETRY_SYS_LOG, "Geometry id mismatch. Check registration logic.");
        return;
    }
