...
void VulkanBackend::create_render_pass() {
    ...
    auto result = _device.createRenderPass(&create_info, _allocator, &_render_pass);
...
void VulkanBackend::create_pipeline() {
    ...
    _pipeline_layout = _device.createPipelineLayout(layout_info, _allocator);
    ...
    auto result = _device.createGraphicsPipeline(VK_NULL_HANDLE, create_info, _allocator);
    if (result.result != vk::Result::eSuccess)
        throw std::runtime_error("Failed to create graphics pipeline.");
    _graphics_pipeline = result.value;

    // Free unused objects
    _device.destroyShaderModule(vertex_shader_module, _allocator);
    _device.destroyShaderModule(fragment_shader_module, _allocator);
}
