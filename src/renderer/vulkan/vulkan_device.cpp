...
VulkanDevice::VulkanDevice(
    ...
    auto physical_device =
        pick_physical_device(vulkan_instance, vulkan_surface);
    ...
    Logger::log(
        RENDERER_VULKAN_LOG, "Vulkan api version\t: ", _info.api_version
    );
    ...
    if (VulkanSettings::graphics_family_required)
        graphics_queue =
            _handle.getQueue(queue_family_indices.graphics_family.value(), 0);
    if (VulkanSettings::present__family_required)
        presentation_queue =
            _handle.getQueue(queue_family_indices.present_family.value(), 0);
    if (VulkanSettings::transfer_family_required)
        transfer_queue =
            _handle.getQueue(queue_family_indices.transfer_family.value(), 0);
    if (VulkanSettings::compute__family_required)
        compute_queue =
            _handle.getQueue(queue_family_indices.compute_family.value(), 0);
}
...
vk::PhysicalDevice VulkanDevice::pick_physical_device(
    const vk::Instance& vulkan_instance, const vk::SurfaceKHR& vulkan_surface
) const {
    ...
    if (devices.size() == 0)
        Logger::fatal(
            RENDERER_VULKAN_LOG, "Failed to find GPUs with Vulkan support."
        );
    ...
    for (const auto& device : devices) {
        auto device_suitability =
            rate_device_suitability(device, vulkan_surface);
...
vk::Device VulkanDevice::create_logical_device(
    ...
    for (auto queue_index : unique_indices) {
        ...
        ); // Number of queues for given family.
    ...
    auto device_features =
        vk::PhysicalDeviceFeatures(VulkanSettings::required_device_features);
    ...
    create_info.setPEnabledExtensionNames(
        VulkanSettings::device_required_extensions
    );
...
PhysicalDeviceInfo VulkanDevice::get_physical_device_info(
    ...
    vk::PhysicalDeviceMemoryProperties device_memory =
    ...
    device_info.max_sampler_anisotropy =
        device_properties.limits.maxSamplerAnisotropy;
    // Maximum number of samples used for Multisample Anti-aliasing (max of the
    // two)
    device_info.framebuffer_color_sample_counts =
    ...
    device_info.get_format_properties = [=](const vk::Format format
                                        ) -> vk::FormatProperties {
...
QueueFamilyIndices VulkanDevice::find_queue_families(
    ...
    for (const auto& queue_family : queue_families) {
        ...
        if (queue_family.queueFlags & vk::QueueFlagBits::eTransfer) {
            ...
            // Most likely to be the one with the minimum number of other
            // functionalities
...
int32 VulkanDevice::rate_device_suitability(
    ...
    if (!queue_family_indices.is_complete(
        ) || // Device must posses all required queue families
        !check_device_extension_support(device
        ) || // Device must support all required extensions
        !device_supports_required_features(device_features
        ) // Device must posses all required features
    )
        return {};
    auto swapchain_support =
        query_swapchain_support_details(device, vulkan_surface);
    if (swapchain_support.formats.empty(
        ) || // Device support at least one format
        swapchain_support.presentation_modes.empty(
        ) // Device support at least one presentation mode
    )
        return {};
    ...
    suitability_score += VulkanSettings::max_texture_size_weight *
...
bool check_device_extension_support(const vk::PhysicalDevice& device) {
    ...
    Set<std::string> required_extensions {
...
bool device_supports_required_features(
    ...
    return (!required_features.robustBufferAccess || features.robustBufferAccess
           ) &&
           (!required_features.fullDrawIndexUint32 ||
           ...
           (!required_features.drawIndirectFirstInstance ||
           ...
           (!required_features.textureCompressionETC2 ||
            features.textureCompressionETC2) &&
           (!required_features.textureCompressionASTC_LDR ||
            features.textureCompressionASTC_LDR) &&
           (!required_features.textureCompressionBC ||
            features.textureCompressionBC) &&
           (!required_features.occlusionQueryPrecise ||
            features.occlusionQueryPrecise) &&
           (!required_features.pipelineStatisticsQuery ||
            features.pipelineStatisticsQuery) &&
           (!required_features.vertexPipelineStoresAndAtomics ||
            features.vertexPipelineStoresAndAtomics) &&
           (!required_features.fragmentStoresAndAtomics ||
            features.fragmentStoresAndAtomics) &&
           (!required_features.shaderTessellationAndGeometryPointSize ||
            features.shaderTessellationAndGeometryPointSize) &&
           (!required_features.shaderImageGatherExtended ||
            features.shaderImageGatherExtended) &&
           (!required_features.shaderStorageImageExtendedFormats ||
            features.shaderStorageImageExtendedFormats) &&
           (!required_features.shaderStorageImageMultisample ||
            features.shaderStorageImageMultisample) &&
           (!required_features.shaderStorageImageReadWithoutFormat ||
            features.shaderStorageImageReadWithoutFormat) &&
           (!required_features.shaderStorageImageWriteWithoutFormat ||
            features.shaderStorageImageWriteWithoutFormat) &&
           (!required_features.shaderUniformBufferArrayDynamicIndexing ||
            features.shaderUniformBufferArrayDynamicIndexing) &&
           (!required_features.shaderSampledImageArrayDynamicIndexing ||
            features.shaderSampledImageArrayDynamicIndexing) &&
           (!required_features.shaderStorageBufferArrayDynamicIndexing ||
           ...
           (!required_features.shaderResourceResidency ||
            features.shaderResourceResidency) &&
           (!required_features.shaderResourceMinLod ||
            features.shaderResourceMinLod) &&
           (!required_features.sparseBinding || features.sparseBinding) &&
           (!required_features.sparseResidencyBuffer ||
            features.sparseResidencyBuffer) &&
           (!required_features.sparseResidencyImage2D ||
            features.sparseResidencyImage2D) &&
           (!required_features.sparseResidencyImage3D ||
            features.sparseResidencyImage3D) &&
           (!required_features.sparseResidency2Samples ||
            features.sparseResidency2Samples) &&
           (!required_features.sparseResidency4Samples ||
            features.sparseResidency4Samples) &&
           (!required_features.sparseResidency8Samples ||
            features.sparseResidency8Samples) &&
           (!required_features.sparseResidency16Samples ||
            features.sparseResidency16Samples) &&
           (!required_features.sparseResidencyAliased ||
            features.sparseResidencyAliased) &&
           (!required_features.variableMultisampleRate ||
