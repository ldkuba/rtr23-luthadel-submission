...
Resource* MaterialLoader::load(const String name) {
    ...
    for (auto setting_line : material_settings) {
        ...
        if (setting.size() != 2) {
            Logger::warning(RESOURCE_LOG, "Potential formatting issue ",
        ...
        if (setting_var.compare(M_SETTING_version) == 0) {
            ...
            if (setting_val.length() > Material::max_name_length)
                Logger::warning(RESOURCE_LOG, "Couldn't load material name at line ",
            ...
            if (str_floats.size() != 4) {
                Logger::warning(RESOURCE_LOG, "Couldn't parse vec4 at line ",
            ...
            Logger::warning(RESOURCE_LOG, " Invalid variable : \"",
